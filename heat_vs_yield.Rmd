...
newDF <- data.frame(rbind(cer, leg)) %>%
  ...
  na.exclude %>%
  filter(accumulated_heat_units < 900)
```
```{r}
# Linear regression
...
```{r}
# More complex model without mutiplying duration and temperature difference between stress and control
...
# It looks like duration alone is the better model based on AIC and BIC but it explains less of the variance (R2)
```
...
leg <- newDF %>% filter(crop=="Leg") %>% filter(accumulated_heat_units <= 250)
# Figure 2 A
