...
ggplot(data = crops, aes(x=accumulated_heat_units_log, y=relative_yield_at_heat, color=gen_type, group=gen_type)) +
  geom_point(size=1) + 
  theme_classic() + 
  scale_color_brewer(2, palette = "Accent") + 
  geom_smooth(method='lm', formula= y~x) +
  xlab("Log10 accumulated heat units (days*degC)") + 
  ylab("Relative yield under heat stress (%)")
ggsave("yield_vs_acc_heat_dummy_new_max500.pdf")

# Check whether crop type affects the response to accumulated heat units
anova(lm(relative_yield_at_heat ~ accumulated_heat_units_log*gen_type, data=crops))
# p < 2.2e-16, adjusted R^2 = 0.2
sctest(crops$relative_yield_at_heat ~ crops$accumulated_heat_units, type = "Chow", point=nrow(leg_dmy))
# p < 1.3e-15, F = 35.3
...
# Testing whether the regressions are significantly different (Chow test)
sctest(pts$relative.reduction.in.PS ~ pts$t.above.optimal, point = nrow(pts[pts$crop_type == "cereals",]), type="Chow")
# p = 0.051, F = 3.1
```
```{r}
